...
# ag_gemm_b200.cu tiles rows and columns in 256-wide blocks only.
TK_GRANULARITY = 256
...
def matched_cutlass_config(col_block: int, num_cta: int) -> CutlassConfig:
    """The candidate whose geometry mirrors what mKernel dispatches."""
    return ((128 * num_cta, col_block), (num_cta, 1), num_cta == 2)
...
def mkernel_schedule(projection: str, m: int) -> tuple[int, int]:
    """Return the (COL_BLOCK, NUM_CTA) the entrypoint dispatches here."""
    try:
        return MKERNEL_SCHEDULE[(projection, m)]
    except KeyError:
        raise ValueError(
            f"no mKernel schedule for {projection} M={m}; add it here and to "
            "the entrypoint switch in ag_gemm_kda_mla.cuh"
        ) from None


def pad_rows(a_local: torch.Tensor, padded_local_m: int) -> torch.Tensor:
...
def pad_cols(b: torch.Tensor, padded_n: int) -> torch.Tensor:
    """Zero-pad B up to padded_n columns."""
    logical_n = b.size(1)
    if logical_n == padded_n:
        return b
    padded = torch.zeros((K, padded_n), device="cuda", dtype=torch.bfloat16)
...
def main() -> int:
    ...
    for (projection, logical_n), m in product(projections, GLOBAL_M):
        ...
        local_m = m // world_size
        col_block, num_cta = mkernel_schedule(projection, m)
        ...
        A_kernel.data_.copy_(pad_rows(A_ref_local, padded_local_m))
        A_local_buf = torch.empty(
            (padded_m, K), device="cuda", dtype=torch.bfloat16
        )
        B_kernel = pad_cols(B_ref, padded_n)
    ...
    for (projection, logical_n), m in product(projections, GLOBAL_M):
        ...
        local_m = m // world_size
        # Each implementation pads to its own tiler's granularity: mKernel to
        # whatever schedule the entrypoint dispatches, TK and CUTLASS to 256,
        # which every one of their candidate tiles divides.
        col_block, num_cta = mkernel_schedule(projection, m)
        mk_local_m = round_up(local_m, 128 * num_cta)
        mk_m = mk_local_m * world_size
        mk_n = round_up(logical_n, col_block)
        tk_local_m = round_up(local_m, TK_GRANULARITY)
        tk_m = tk_local_m * world_size
        tk_n = round_up(logical_n, TK_GRANULARITY)
        cutlass_n = round_up(logical_n, TK_GRANULARITY)
        cutlass_config_match = matched_cutlass_config(col_block, num_cta)
        ...
        if cutlass_ok:
            try:
                # Let cutlass deal with rounding up itself
        ...
        # One A shard and one B per kernel, each at the shape that kernel
        # needs. They coincide on most shapes, so share the tensor when the
        # paddings agree rather than allocating a second copy.
        A_mk_local = pad_rows(A_ref_local, mk_local_m)
        A_tk_local = (
            A_mk_local
            if tk_local_m == mk_local_m
            else pad_rows(A_ref_local, tk_local_m)
        )
        B_kernel = pad_cols(B_ref, mk_n)
        B_tk = B_kernel if tk_n == mk_n else pad_cols(B_ref, tk_n)
        ...
        A_kernel.data_.copy_(A_mk_local)
        A_local_buf = torch.empty(
            (mk_m, K), device="cuda", dtype=torch.bfloat16
        )
        C_kernel = torch.zeros(
            (mk_m, mk_n), device="cuda", dtype=torch.bfloat16
        )
        ...
        if mk_m == m:
            ...
            C_padded = torch.zeros(
                (m, mk_n), device="cuda", dtype=torch.bfloat16
            )

        # Every rank must finish filling its DistBuffer before any rank
        # launches, since the kernel opens by pulling every peer's shard.
        torch.cuda.synchronize()
        dist.barrier()
        ...
        if tk_ok:
            ...
            if not shape_vote.item():
                ...
                if is_chief:
                    print(
                        f"  [skip] ThunderKittens {projection} M={tk_m} "
                        f"N={tk_n}: "
                ...
                if not check_close(
                    ...
                    raise RuntimeError(
                        ...
                        f"{projection} M={m} padded_m={tk_m} N={tk_n}"
                    )
        ...
        if is_chief:
            print(
                f"{projection} M={m} local_m={local_m} N={logical_n} "
                f"mkernel {mk_m}x{mk_n} (COL_BLOCK={col_block} "
                f"NUM_CTA={num_cta})  tk {tk_m}x{tk_n}  "
                ...
                f"TFLOP/s\n"
                f"  {f'cuBLAS N={mk_n}':<26} {cublas_padded_ms:8.3f} ms  "
        ...
        del A_ref_local, A_ref, B_ref, C_ref, A_mk_local, A_tk_local
